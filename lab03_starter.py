...
# Done: Use list() and range to create a list of dic values ([1, 2, 3, 4, 5, 6]) instead of the current hard-coded dice values.
...
#Done: Use a for loop with the in keyword to iterate over the weapons array and display the available weapons to the player.
...
#Done sk the player to input an integer between 1 and 6 for their combat strength (combatStrength) and monster combat strength (mCombatStrength). Validate the input to ensure it's an integer between 1 and 6. If the input is invalid, prompt the user again.
